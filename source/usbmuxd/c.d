...
version (Windows) {
    version (MinGW) {
        enum libusbmuxd = LibImport("libusbmuxd-2.0.dll");
    } else {
        enum libusbmuxd = LibImport("usbmuxd.dll");
    }
} else version (OSX) {
    enum libusbmuxd = LibImport("libusbmuxd-2.0.7.dylib", "libusbmuxd-2.0.6.dylib");
} else {
    enum libusbmuxd = LibImport("libusbmuxd-2.0.so.7", "libusbmuxd-2.0.so.6", "libusbmuxd.so.2", "libusbmuxd-2.0.so");
}
 ...
 */
struct usbmuxd_device_info_t
{
    uint handle;
    uint product_id;
    char[44] udid;
    usbmux_connection_type conn_type;
    char[200] conn_data;
}
 ...
 */
alias usbmuxd_event_cb_t = void function (const(usbmuxd_event_t)* event, void* user_data);

/**
 * Subscription context type.
 */
struct usbmuxd_subscription_context;
alias usbmuxd_subscription_context_t = usbmuxd_subscription_context*;
 ...
 *    or a negative value on error.
 */
int usbmuxd_get_device (const(char)* udid, usbmuxd_device_info_t* device, usbmux_lookup_options options);
 ...
 *    value on error.
 */
int usbmuxd_connect (const uint handle, const ushort tcp_port);
 ...
 */
int usbmuxd_send (int sfd, const(char)* data, uint len, uint* sent_bytes);
 ...
 * @return 0 on success, a negative errno value otherwise.
 */
int usbmuxd_recv_timeout (int sfd, char* data, uint len, uint* recv_bytes, uint timeout);
 ...
 * @return 0 on success, a negative errno value otherwise.
 */
int usbmuxd_recv (int sfd, char* data, uint len, uint* recv_bytes);
 ...
 */
int usbmuxd_read_pair_record (const(char)* record_id, char** record_data, uint* record_size);
 ...
 * @return 0 on success, a negative error value otherwise.
 */
int usbmuxd_save_pair_record (const(char)* record_id, const(char)* record_data, uint record_size);
 ...
 * @return 0 on success, a negative error value otherwise.
 */
int usbmuxd_save_pair_record_with_device_id (const(char)* record_id, uint device_id, const(char)* record_data, uint record_size);
