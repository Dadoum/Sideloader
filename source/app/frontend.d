...
shared abstract class Frontend {
    __gshared Device device;
    __gshared ADI adi;

    abstract string configurationPath();
    abstract int run(string[] args);

    final void initializeADI() {
        auto log = getLogger();
    ...
    final bool downloadAndInstallDeps(bool delegate(float progress) downloadCallback) {
        auto log = getLogger();
