...
alias idevice_t = idevice_private*; /**< The device handle. */

struct idevice_connection_private;
alias idevice_connection_t = idevice_connection_private*; /**< The connection handle. */
...
alias idevice_info_t = idevice_info*;
...
{
    idevice_event_type event; /**< The event type. */
    const(char)* udid; /**< The device unique id. */
    idevice_connection_type conn_type; /**< The connection type. */
}

/* event callback function prototype */
/** Callback to notifiy if a device was added or removed. */
alias idevice_event_cb_t = void function (const(idevice_event_t)* event, void* user_data);
 ...
 * @return IDEVICE_E_SUCCESS if ok, otherwise an error code.
 */
idevice_error_t idevice_connect (idevice_t device, ushort port, idevice_connection_t* connection);
 ...
 * @return IDEVICE_E_SUCCESS if ok, otherwise an error code.
 */
idevice_error_t idevice_connection_send (idevice_connection_t connection, const(char)* data, uint len, uint* sent_bytes);
 ...
 * @return IDEVICE_E_SUCCESS if ok, otherwise an error code.
 */
idevice_error_t idevice_connection_receive_timeout (idevice_connection_t connection, char* data, uint len, uint* recv_bytes, uint timeout);
 ...
 * @return IDEVICE_E_SUCCESS if ok, otherwise an error code.
 */
idevice_error_t idevice_connection_receive (idevice_connection_t connection, char* data, uint len, uint* recv_bytes);
 ...
 *     enabled and does no further error checking on cleanup.
 */
idevice_error_t idevice_connection_disable_bypass_ssl (idevice_connection_t connection, ubyte sslBypass);
 ...
 */
idevice_error_t idevice_get_handle (idevice_t device, uint* handle);
